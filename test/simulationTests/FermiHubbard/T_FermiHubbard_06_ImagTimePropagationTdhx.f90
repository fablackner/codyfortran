program T_FermiHubbard_06_ImagTimePropagationTdhx
  use M_Utils_Types
  use M_Utils_Say
  use M_Utils_Json
  use M_Utils_Actions
  ...
  real(R64)    :: convThresh
  real(R64)    :: outputStep
  real(R64)    :: time = 0.0_R64  ! Current simulation time
  real(R64)    :: timeStep
  integer      :: nTimeSteps
  integer(I32) :: iStep, innerStep
  ...
  jsonFileName = "test/simulationTests/FermiHubbard/T_FermiHubbard_06_ImagTimePropagationTdhx.json"
  ...
  call check(error, energyNew, -7.0_R64, thr=1e-7_R64)

  if (allocated(error)) error stop "T_FermiHubbard_ImagTimePropagationTdhx failure"
