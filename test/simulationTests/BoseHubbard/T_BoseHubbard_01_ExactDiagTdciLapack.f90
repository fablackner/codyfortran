program T_BoseHubbard_01_ExactDiagTdciLapack
  use M_Utils_Types
  use M_Utils_Say
  use M_Utils_Json
  use M_Utils_Actions
  ...
  jsonFileName = "test/simulationTests/BoseHubbard/T_BoseHubbard_01_ExactDiagTdciLapack.json"
  ...
  if (DiagonalizerList(1) % e % nFound .eq. 0) then
    error stop "no eigenvalues computed"
  end if
  ...
  call check(error, energy, -14.60164824_R64, thr=1e-7_R64)
  if (allocated(error)) error stop "T_BoseHubbard_ExactDiagTdci failure"
